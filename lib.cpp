...
#define COUNT_WITH_TYPE(TOKEN_LC, TOKEN_CAP, PATTERN_TY)                                           \
	size_t count_##TOKEN_LC(const FmIndex##TOKEN_CAP* fm_index, const PATTERN_TY* pattern,         \
	                        size_t pattern_len) {                                                  \
...
#define LOCATE_WITH_TYPE(TOKEN_LC, TOKEN_CAP, PATTERN_TY)                                          \
	LocateResult* locate_##TOKEN_LC(const FmIndex##TOKEN_CAP* fm_index, const PATTERN_TY* pattern, \
	                                size_t pattern_len) {                                          \
...
#define FM_INDEX(TOKEN_LC, TOKEN_CAP, PATTERN_TY)                                                  \
	CONSTRUCT_FM_WITH_TYPE(TOKEN_LC, TOKEN_CAP)                                                    \
	DESTRUCT_FM_WITH_TYPE(TOKEN_LC, TOKEN_CAP)                                                     \
	COUNT_WITH_TYPE(TOKEN_LC, TOKEN_CAP, PATTERN_TY)                                               \
	SIZE_IN_BYTES_WITH_TYPE(TOKEN_LC, TOKEN_CAP)                                                   \
	LOCATE_WITH_TYPE(TOKEN_LC, TOKEN_CAP, PATTERN_TY)

FM_INDEX(byte32, Byte32, uint8_t);
FM_INDEX(int32, Int32, uint64_t);
FM_INDEX(byte64, Byte64, uint8_t);
FM_INDEX(int64, Int64, uint64_t);
